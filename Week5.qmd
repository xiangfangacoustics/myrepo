...
Londonborough <- st_read("/Users/xfang/Desktop/UCL PHD/PGTA/CASA GIS/Week5/statistical-gis-boundaries-london/ESRI/London_Borough_Excluding_MHW.shp")%>%
  st_transform(., 27700)
```
...
## World cities UK outline and London Borough data

```{r}
Worldcities <- st_read("/Users/xfang/Desktop/UCL PHD/PGTA/CASA GIS/Week5/World_Cities/World_Cities.shp") %>%
  st_transform(., 27700)
UK_outline <- st_read("/Users/xfang/Desktop/UCL PHD/PGTA/CASA GIS/Week5/gadm41_GBR_shp/gadm41_GBR_0.shp") %>%
  st_transform(., 27700)
Londonborough <- st_read("/Users/xfang/Desktop/UCL PHD/PGTA/CASA GIS/Week5/statistical-gis-boundaries-london/ESRI/London_Borough_Excluding_MHW.shp")%>%
  st_transform(., 27700)
```
...
# Create the map with only the default OSM basemap
map <- leaflet(Joined) %>%
  # Add only the default OpenStreetMap basemap
  ...
  # Add a legend for the accommodation count
  addLegend(pal = pal2, values = ~`Hotel count`,
            group = c("Airbnb", "Hotels"),
            position = "bottomleft",
